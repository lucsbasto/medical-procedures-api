...
export function logRequest(req: Request): string {
  const queryMasked = new URLSearchParams(
    maskRecursive(req.query) as Record<string, any>,
  ).toString();
...
export function logResponseException(
  ...
  const reponseLog =
    typeof response === 'string'
      ? response
      : { ...response, metadata: maskRecursive(exception.getMetadata()) };
